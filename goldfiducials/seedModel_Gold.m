function [PD, dB, T2] = seedModel_Gold (xSpace, ySpace, zSpace, R, voxelSize, B0)

[x,y,z] = meshgrid(xSpace,ySpace,zSpace);

susAu = -34e-6; %https://www-mrsrl.stanford.edu/studygroup/2/Files/Schenck_susceptibility.pdf
susBackground = -9e-6;
...
if (~isempty(R))
    ...
    % Cylinder
    dist1 = sqrt(ps(:,2).^2 + ps(:,1).^2) - 0.5;
    dist2 = abs(ps(:,3)) - 5/2;

    dist = max(dist1,dist2);
    H = reshape(1./(1 + exp(-2*k*dist)), size(PD));
    susceptibility = susceptibility.*H + (1-H).*susAu;
    PD = PD.*H;
end
